...
async function fromBacklog(config) {
  ...
  while (page.hasNext()) {
    ...
    page.elements.forEach(element => {
      const newMdFile = genarateMdFileName(element.docId, element.title);
      const doc = {
        file: newMdFile,
 ...
 */
function localMdIndex() {
  if (_localMdIndex) {
    return _localMdIndex;
  }
  const files = glob.sync(`${config.backlog_md_dir}/**/*.md`);
  const index = {};
  files.forEach(file => {
    const doc = md.read(file);
    delete doc.fm.body;
...
function convIssueToPostElement(issue) {
  ...
  const mdFile = genarateMdFileName(post.docId, post.title);
  post.content = md.convToNormalMd(post.content, post.attachments, mdFile, config.backlog_attachment_dir);
  return post;
}
...
function genarateMdFileName(docId, title) {
  const localMds = localMdIndex();
  if (localMds[docId]) {
    return localMds[docId].file;
  }
  const slug = slugger.slug(`${docId} ${title}`);
  return `${config.backlog_md_dir}/${slug}.md`;
}
