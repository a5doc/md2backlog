...
async function fromBacklog(config) {
  ...
  while (page.hasNext()) {
    ...
    page.elements.forEach(element => {
      const newMdFile = md.genarateMdFileName(element.docId, element.title, config.backlog_md_dir);
      const doc = {
        file: newMdFile,
...
async function convIssueToPostElement(issue) {
  ...
  post.content = await md.convToNormalMd(post.content, post.attachments, mdFile, config.backlog_md_dir, config.backlog_attachment_dir, config.backlog_project_key);
  return post;
}
